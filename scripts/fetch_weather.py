#!/usr/bin/env python3
"""
fetch_weather.py
----------------
Open-Meteo → GeoJSON + CSV + COG (Cloud Optimized GeoTIFF) + GeoPackage.

COG = lisibles directement dans QGIS via :
  Couche > Ajouter raster > /vsicurl/https://raw.githubusercontent.com/...tif

GeoPackage = toutes les couches vectorielles en un seul fichier.
"""

import csv
import json
import os
import sys
import tempfile
import time
from datetime import datetime, timezone

import numpy as np
import requests
import scipy.ndimage as ndimage
from scipy.interpolate import RegularGridInterpolator

try:
    import rasterio
    from rasterio.enums import Resampling
    from rasterio.shutil import copy as rio_copy
    from rasterio.transform import from_bounds
    HAS_RASTERIO = True
except ImportError:
    HAS_RASTERIO = False

try:
    import fiona
    from shapely.geometry import shape, Point, mapping
    HAS_FIONA = True
except ImportError:
    HAS_FIONA = False

ZONES = {
    "france": {
        "bbox": (-5.14, 41.33, 9.56, 51.09),
        "resolution": 0.25,
        "raster_size": (600, 400),
        "smooth_sigma": 3.0,
    },
    "haute-marne": {
        "bbox": (4.70, 47.50, 5.95, 48.65),
        "resolution": 0.05,
        "raster_size": (600, 600),
        "smooth_sigma": 4.0,
    },
}

VARIABLES = {
    "temperature_2m":  {"label": "Température (°C)",        "unit": "°C"},
    "precipitation":   {"label": "Précipitations (mm/h)",   "unit": "mm/h"},
    "cloud_cover":     {"label": "Couverture nuageuse (%)", "unit": "%"},
    "wind_speed_10m":  {"label": "Vitesse du vent (m/s)",   "unit": "m/s"},
}

OUTPUT_DIR    = "data/weather"
OPENMETEO_URL = "https://api.open-meteo.com/v1/forecast"
CHUNK_SIZE    = 50
RETRY_DELAY   = 5
NODATA        = -9999.0
LAST_UPDATE   = "data/last_update.json"


# ── Grille & API ────────────────────────────────────────────────────────────

def generate_grid(bbox, resolution):
    lon_min, lat_min, lon_max, lat_max = bbox
    lons = np.arange(lon_min, lon_max + resolution * 0.5, resolution)
    lats = np.arange(lat_min, lat_max + resolution * 0.5, resolution)
    g_lon, g_lat = np.meshgrid(lons, lats)
    return g_lon.flatten(), g_lat.flatten(), lons, lats


def fetch_chunk(lats, lons, retries=3):
    params = {
        "latitude":  ",".join(f"{v:.4f}" for v in lats),
        "longitude": ",".join(f"{v:.4f}" for v in lons),
        "current":   ",".join(VARIABLES.keys()),
        "wind_speed_unit": "ms",
        "timezone":  "Europe/Paris",
        "forecast_days": 1,
    }
    for attempt in range(retries):
        try:
            r = requests.get(OPENMETEO_URL, params=params, timeout=60)
            r.raise_for_status()
            d = r.json()
            return [d] if isinstance(d, dict) else d
        except requests.RequestException as e:
            if attempt < retries - 1:
                print(f"   ⚠️  T{attempt+1}/{retries}: {e}. Retry {RETRY_DELAY}s...")
                time.sleep(RETRY_DELAY)
            else:
                raise


def extract_records(api_results):
    out = []
    for r in api_results:
        cur = r.get("current", {})
        rec = {"lat": round(r["latitude"], 4), "lon": round(r["longitude"], 4)}
        for v in VARIABLES:
            rec[v] = cur.get(v)
        out.append(rec)
    return out


# ── GeoJSON ─────────────────────────────────────────────────────────────────

def save_geojson(records, zone_name, timestamp):
    features = []
    for r in records:
        if r.get("temperature_2m") is None:
            continue
        props = {k: v for k, v in r.items() if k not in ("lat", "lon")}
        props["last_updated"] = timestamp
        features.append({
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [r["lon"], r["lat"]]},
            "properties": props,
        })
    path = os.path.join(OUTPUT_DIR, f"{zone_name}_weather.geojson")
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"type": "FeatureCollection",
                   "metadata": {"last_updated": timestamp, "zone": zone_name,
                                "nb_points": len(features)},
                   "features": features}, f, ensure_ascii=False, indent=2)
    print(f"✓  {path} ({len(features)} points)")
    return features


# ── CSV ──────────────────────────────────────────────────────────────────────

def save_csv(records, zone_name, timestamp):
    valid = [r for r in records if r.get("temperature_2m") is not None]
    if not valid:
        return
    path   = os.path.join(OUTPUT_DIR, f"{zone_name}_weather.csv")
    fields = ["lon", "lat"] + list(VARIABLES.keys()) + ["last_updated"]
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
        for r in valid:
            row = {k: (round(r[k], 5) if isinstance(r.get(k), float) else r.get(k))
                   for k in fields if k != "last_updated"}
            row["last_updated"] = timestamp
            w.writerow(row)
    print(f"✓  {path} ({len(valid)} lignes)")


# ── Interpolation (RegularGridInterpolator + gaussien) ───────────────────────

def interpolate(records, var_name, bbox, raster_size, lons_src, lats_src, sigma):
    valid = [(r["lon"], r["lat"], r[var_name])
             for r in records if r.get(var_name) is not None]
    if len(valid) < 4:
        return None

    n_lat, n_lon = len(lats_src), len(lons_src)
    grid_src = np.full((n_lat, n_lon), np.nan, dtype=np.float64)
    for lon, lat, val in valid:
        i = int(np.argmin(np.abs(lats_src - lat)))
        j = int(np.argmin(np.abs(lons_src - lon)))
        grid_src[i, j] = val

    # Remplissage NaN résiduels (nearest-neighbor)
    nan_mask = np.isnan(grid_src)
    if nan_mask.any():
        from scipy.ndimage import distance_transform_edt
        _, idx = distance_transform_edt(nan_mask, return_indices=True)
        grid_src[nan_mask] = grid_src[idx[0][nan_mask], idx[1][nan_mask]]

    interp = RegularGridInterpolator(
        (lats_src, lons_src), grid_src,
        method="linear", bounds_error=False, fill_value=None)

    lon_min, lat_min, lon_max, lat_max = bbox
    w, h = raster_size
    tg_lon = np.linspace(lon_min, lon_max, w)
    tg_lat = np.linspace(lat_min, lat_max, h)
    g2_lon, g2_lat = np.meshgrid(tg_lon, tg_lat)
    pts = np.stack([g2_lat.ravel(), g2_lon.ravel()], axis=-1)

    grid_vals = interp(pts).reshape(h, w).astype(np.float32)
    return ndimage.gaussian_filter(grid_vals, sigma=sigma)


# ── COG (Cloud Optimized GeoTIFF) ────────────────────────────────────────────

def save_cog(grid_vals, var_name, zone_name, bbox, raster_size, timestamp):
    """
    Génère un Cloud Optimized GeoTIFF (COG) avec overviews.
    Lisible directement dans QGIS via :
      /vsicurl/https://raw.githubusercontent.com/.../haute-marne_temperature_2m.tif
    """
    if not HAS_RASTERIO or grid_vals is None:
        return

    lon_min, lat_min, lon_max, lat_max = bbox
    w, h = raster_size
    arr       = np.where(np.isnan(grid_vals), NODATA, grid_vals).astype(np.float32)
    arr_flip  = np.flipud(arr)   # nord en haut
    transform = from_bounds(lon_min, lat_min, lon_max, lat_max, w, h)
    path      = os.path.join(OUTPUT_DIR, f"{zone_name}_{var_name}.tif")

    # 1. Fichier temporaire avec overviews
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".tif")
    os.close(tmp_fd)
    try:
        with rasterio.open(tmp_path, "w",
                driver="GTiff", height=h, width=w, count=1,
                dtype=np.float32, crs="EPSG:4326",
                transform=transform, nodata=NODATA) as dst:
            dst.write(arr_flip, 1)
            dst.build_overviews([2, 4, 8, 16], Resampling.nearest)
            dst.update_tags(ns="rio_overview", resampling="nearest")
            dst.update_tags(
                variable=var_name,
                label=VARIABLES[var_name]["label"],
                unit=VARIABLES[var_name]["unit"],
                zone=zone_name,
                last_updated=timestamp,
                source="Open-Meteo CC BY 4.0",
            )

        # 2. Copie en COG (tuilé, deflate, overviews copiés)
        with rasterio.open(tmp_path) as src:
            rio_copy(src, path,
                     copy_src_overviews=True,
                     driver="GTiff",
                     tiled=True,
                     compress="deflate",
                     blockxsize=512,
                     blockysize=512)
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)

    size_kb = os.path.getsize(path) // 1024
    print(f"✓  {path} (COG, {w}×{h} px, {size_kb} Ko)")


# ── GeoPackage (toutes couches vectorielles en un fichier) ───────────────────

def save_gpkg(all_features: dict[str, list], timestamp: str):
    """
    Crée un GeoPackage contenant toutes les couches de points météo.
    Lisible dans QGIS : Couche > Ajouter vecteur > GeoPackage > URL raw GitHub.
    """
    if not HAS_FIONA:
        print("   ⚠️  fiona non disponible — GeoPackage désactivé")
        return

    path = os.path.join(OUTPUT_DIR, "weather.gpkg")

    schema = {
        "geometry": "Point",
        "properties": {
            "zone":            "str",
            "temperature_2m":  "float",
            "precipitation":   "float",
            "cloud_cover":     "float",
            "wind_speed_10m":  "float",
            "last_updated":    "str",
        },
    }

    with fiona.open(path, "w", driver="GPKG", crs="EPSG:4326", schema=schema) as dst:
        for zone_name, features in all_features.items():
            for feat in features:
                props = feat["properties"]
                dst.write({
                    "geometry": feat["geometry"],
                    "properties": {
                        "zone":           zone_name,
                        "temperature_2m": props.get("temperature_2m"),
                        "precipitation":  props.get("precipitation"),
                        "cloud_cover":    props.get("cloud_cover"),
                        "wind_speed_10m": props.get("wind_speed_10m"),
                        "last_updated":   props.get("last_updated", timestamp),
                    },
                })

    size_kb = os.path.getsize(path) // 1024
    print(f"✓  {path} (GeoPackage, {size_kb} Ko)")


# ── last_update.json ─────────────────────────────────────────────────────────

def update_last_update(zone_name: str, timestamp: str, nb_points: int):
    os.makedirs(os.path.dirname(LAST_UPDATE), exist_ok=True)
    try:
        with open(LAST_UPDATE, encoding="utf-8") as f:
            data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        data = {}

    data[f"weather_{zone_name}"] = {
        "timestamp_utc": timestamp,
        "source": "Open-Meteo",
        "nb_points": nb_points,
    }
    data["_last_any_update"] = timestamp

    with open(LAST_UPDATE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


# ── Traitement d'une zone ────────────────────────────────────────────────────

def process_zone(zone_name, cfg, timestamp):
    print(f"\n📍 Zone : {zone_name}  (résolution {cfg['resolution']}°)")
    lons_flat, lats_flat, lons_src, lats_src = generate_grid(cfg["bbox"], cfg["resolution"])
    total   = len(lons_flat)
    n_chunks = (total + CHUNK_SIZE - 1) // CHUNK_SIZE
    print(f"   Grille : {total} points → {n_chunks} requêtes")

    all_records = []
    for i in range(0, total, CHUNK_SIZE):
        chunk_lats = lats_flat[i: i + CHUNK_SIZE]
        chunk_lons = lons_flat[i: i + CHUNK_SIZE]
        n_end = min(i + CHUNK_SIZE, total)
        print(f"   Chunk {i+1}–{n_end}...")
        results = fetch_chunk(chunk_lats, chunk_lons)
        all_records.extend(extract_records(results))
        if n_end < total:
            time.sleep(0.2)

    features = save_geojson(all_records, zone_name, timestamp)
    save_csv(all_records, zone_name, timestamp)

    for var_name in VARIABLES:
        grid = interpolate(all_records, var_name,
                           cfg["bbox"], cfg["raster_size"],
                           lons_src, lats_src, cfg["smooth_sigma"])
        save_cog(grid, var_name, zone_name, cfg["bbox"], cfg["raster_size"], timestamp)

    update_last_update(zone_name, timestamp, len(features))
    return features


# ── Main ─────────────────────────────────────────────────────────────────────

def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

    all_features = {}
    errors = []
    for zone_name, cfg in ZONES.items():
        try:
            feats = process_zone(zone_name, cfg, timestamp)
            all_features[zone_name] = feats
        except Exception as e:
            msg = f"Erreur {zone_name} : {e}"
            print(f"❌ {msg}", file=sys.stderr)
            errors.append(msg)

    # GeoPackage global (toutes zones)
    if all_features:
        save_gpkg(all_features, timestamp)

    with open(os.path.join(OUTPUT_DIR, "metadata.json"), "w", encoding="utf-8") as f:
        json.dump({
            "last_updated": timestamp,
            "source": "Open-Meteo (https://open-meteo.com)",
            "license": "CC BY 4.0",
            "zones": {k: {"bbox": v["bbox"], "resolution_deg": v["resolution"]}
                      for k, v in ZONES.items()},
            "variables": {k: v for k, v in VARIABLES.items()},
            "outputs": ["*.geojson", "*.csv", "*.tif (COG)", "weather.gpkg"],
            "errors": errors,
        }, f, ensure_ascii=False, indent=2)

    if errors:
        print(f"\n⚠️  {len(errors)} erreur(s)", file=sys.stderr)
        sys.exit(1)
    print("\n✅ Météo OK.")


if __name__ == "__main__":
    main()
